...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-4 z-50 flex w-full flex-col items-center">
      ...
      lg:rounded-full lg:shadow-lg bg-yellow-600">
        <img src={logo} alt="logo" width={80} height={22}/>
        <div className="hidden space-x-6 lg:flex">
          {LINKS.map((link, index) => (
             
            <a key={index} href={`#${link.targetId}}`} className={'text-sm ${index !== 0 ? "border-1-2 text-white border-neutral-300/20" pl-2 : ""}hover:opacity-50'} onClick={(e) => handleScroll
